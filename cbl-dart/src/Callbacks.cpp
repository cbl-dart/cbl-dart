...
CallbackIsolate::CallbackIsolate(Dart_Handle handle,
                                 Dart_Port sendPort) : sendPort_(sendPort)
{
...
void CallbackIsolate::finalizer(void *dart_callback_data,
                                void *peer)
{
    auto &self = *reinterpret_cast<CallbackIsolate *>(peer);

    // Clean up the callbacks which have not been unregistered before the Isolate died.
...
void CallbackIsolate::setCallbackFinalizer(CallbackId callbackId,
                                           void *context,
                                           CallbackFinalizer finalizer)
{
    // We are not synchronizing access here because Isolates are single threaded and
    // the only callers of this api.
                                      ...
                                      bool runFinalizer)
{
    ...
    {
        if (runFinalizer)
            finalizer->first(callbackId, finalizer->second);
...
CallbackCall::CallbackCall(const std::function<CallbackResultHandler> &resultHandler)
    : resultHandler(&resultHandler)
{
    receivePort = Dart_NewNativePort_DL("CallbackCall", &CallbackCall::messageHandler, false);
};
...
void CallbackCall::execute(CallbackId callbackId,
                           Dart_CObject *arguments)
{
    ...
    // Pointer to the callback to invoke, through which the Dart side can identify the callback.
    ...
    // Pointer to this call, which is sent back by the Dart side in the result message.
    // This is how we get a reference to this call in the message handler.
    // Only necessary if the caller is interested in the result.
    ...
    Dart_CObject *messageValues[] = {&callbackId_, &responsePort, &callPointer, arguments};
