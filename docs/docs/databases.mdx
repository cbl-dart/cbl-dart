...
- The database is created in a default location unless you specify a directory
  ...
  for the default location on each platform.
...
You are advised to incorporate the closing of all open databases into your
application workflow.

Closing a database is simple, just use `api|Database.close`. See [Example 2](#).
This also closes active replications, listeners, and-or live queries connected
to the database.

:::note

Closing a database soon after starting a replication involving it can cause an
exception as the asynchronous replicator start may not yet be connected.
...
Database Full Sync prevents the loss of transactional data due to an unexpected
system crash or loss of power. This feature is not enabled by default and must
be manually set in your database configuration using
`api|DatabaseConfiguration.fullSync`.

:::caution

Database Full Sync is a safe method to prevent data loss but will incur a
significant degradation of performance.

:::

<CodeExample id={3} title="Enable Database Full Sync">
...
Once a database is created, its configuration is immutable — modifying the
`api|DatabaseConfiguration` afterwards has no effect on the existing instance.
...
database is opened - see [Example 4](#).

<CodeExample id={4} title="Opening an Encrypted Database">
...
<CodeExample id={5} title="Increase level of Database Log Messages">
