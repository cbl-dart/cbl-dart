...
final collectionInCustomScope = await database.collection(
  'my-collection',
  'my-scope',
);
```
...
```dart
final collectionInDefaultScope = await database.createCollection(
  'my-collection',
);
final collectionInCustomScope = await database.createCollection(
  'my-collection',
  'my-scope',
);
```
...
```dart
final doc = MutableDocument({'name': 'Alice', 'age': 29});
...
```dart
final doc = MutableDocument(id: 'ali', {'name': 'Alice', 'age': 29});
...
final config = ReplicatorConfiguration(
  target: UrlEndpoint(Uri.parse('http://localhost:4984/my-database')),
