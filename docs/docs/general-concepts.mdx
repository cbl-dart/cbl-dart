...
/// returns its value.
Future<int> runCountQuery(Query query) async {
  final resultSet = await query.execute();
  final results = await resultSet.allResults();
  // Returns the first column of the first row.
  return results[0].integer(0);
}
```
...
```dart main
final token = await collection.addChangeListener((change) {});
