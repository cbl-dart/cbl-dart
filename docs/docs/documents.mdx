...
final mutableDoc = MutableDocument(null, id: 'hotel::1');
```
...
final mutableDocumemt = MutableDocument(null, id: 'hotel::1')
...
where the document ID is randomly generated by the database. To create a
document with a specific ID, pass the `id` parameter.
...
```dart
final document = MutableDocument(null)
