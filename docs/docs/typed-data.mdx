...
export const CblGeneratorInstallCommand = () => {
  const version = usePubPackageVersion('cbl_generator')
  return (
    <CodeBlock language="bash">{`flutter pub add --dev cbl_generator:${version} build_runner`}</CodeBlock>
  )
}
...
1. Open an instance of the typed database and use it:
   ...
   Future<void> useTypedDatabase() async {
