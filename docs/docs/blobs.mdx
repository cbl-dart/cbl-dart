...
- Couchbase Lite <br /> Blobs can be arbitrarily large. They are only read on
  demand, not when you load the document.
...
// Create the blob with a MIME type and the image data.
final blob = Blob.fromData('image/jpeg', data);

// Add the blob to the document using 'avatar' as the property key.
// Saving the document generates a digest (a SHA-1 hash of the content)
// stored in the blob's metadata — see Figure 1.
doc.setBlob(blob, key: 'avatar');
await collection.saveDocument(doc);

// Later, retrieve the blob from the document by key.
...
When you delete a document that references a blob, the blob file itself is not
immediately removed from the database. The blob file is kept in the database
until you compact it.

To remove orphaned blob files, perform database maintenance using the compact
option — see [Example 2](#).

<CodeExample id={2} title="Compact Database to Remove Orphaned Blobs">
