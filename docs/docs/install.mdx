...
export const CblInstallCommand = () => {
  const version = usePubPackageVersion('cbl')
  return <CodeBlock language="bash">{`dart pub add cbl:${version}`}</CodeBlock>
}
...
If you enabled vector search in `pubspec.yaml`, enable it explicitly after
initialization and before opening a database that uses it:
...
See [Working with Vector Search](search/working-with-vector-search.mdx) for
details.
...
your app and call it after initializing Couchbase Lite:
