...
Couchbase Lite's `api|QueryBuilder` API includes conditional functions for
handling NULL and MISSING values.

- `api|Function_.ifMissingOrNull`

  Returns the first non-NULL and non-MISSING value from the provided
  expressions, or NULL if all values are NULL or MISSING. This function is
  equivalent to the SQL++ IFMISSINGORNULL function. It requires at least 2
  expressions.

  <CodeExample id={18} title="Using Function_.ifMissingOrNull()">

  This example shows how to use `ifMissingOrNull` to provide fallback values
  when properties might be missing or null.
    ...
    .select(
      ...
      SelectResult.expression(
        Function_.ifMissingOrNull(
          Expression.property('preferredName'),
