...
Use `api|Database.createQuery` to define a query through an SQL++ string. Then
run the query using the `api|Query.execute()` method.

<CodeExample id={1} title="Running an SQL++ Query">
...
2. Select all properties from the `user` data source. Give the object an alias
   of `data`.
...
from the `airlines` collection using the document ID (`id`) of the _airline_
document and the `airlineId` property of the _route_ document.
...
<CodeExample id={51} title="Running an SQL++ Query">
...
await query.setParameters(Parameters({'country': 'France'}));
final resultSet = await query.execute();
```
