...
<CodeExample id={9} title="where with tolower function">
...
<CodeExample id={11} title="where with tolower function">
...
Consider fetching details lazily. You could break complex queries into components. Returning just the doc-ids, then process the array of doc-ids using either the Document API or a query thats uses the array of doc-ids to return information.
