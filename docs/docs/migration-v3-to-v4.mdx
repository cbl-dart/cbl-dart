...
```dart
// Before (v3)
final config = LogFileConfiguration(directory: logDir, maxRotateCount: 5);

// After (v4)
final config = LogFileConfiguration(directory: logDir, maxKeptFiles: 5);
```
