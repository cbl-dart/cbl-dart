...
In v4, configure the edition in your workspace root `pubspec.yaml`:
...
x86-64. 32-bit ARM is not supported.
...
4. **Configure edition** (if using Enterprise) in your workspace root
   `pubspec.yaml` under `hooks.user_defines.cbl`.
5. **Update imports**:
...
8. **Run your app** -- native libraries will be built automatically on first
   run.
