...
8. **Update certificate fields** on `ReplicatorConfiguration` -- wrap
   `pinnedServerCertificate` bytes with `DerData` or `PemData`, and
   `trustedRootCertificates` bytes with `PemData`.
9. **Rename `maxRotateCount`** to `maxKeptFiles` in any `LogFileConfiguration`
   usage.
10. **Update query result checks** that compare boolean expressions against `1`
    or `0` -- they now return `true`/`false`.
11. **Remove hardcoded revision IDs** -- revision IDs are no longer
    deterministic.
12. **Update conflict handling** -- `saveDocument` and `deleteDocument` with
    `failOnConflict` now throw `DatabaseException` instead of returning `false`.
13. **Migrate Database document/index/listener APIs** to use `defaultCollection`
    methods instead.
14. **Migrate ReplicatorConfiguration** -- replace `database` parameter with
    `addCollection`, move `channels`/`documentIds`/filters/resolvers into
    `CollectionConfiguration`.
15. **Migrate Replicator pending document APIs** -- replace
    `pendingDocumentIds`/`isDocumentPending` with
    `pendingDocumentIdsInCollection`/`isDocumentPendingInCollection`.
16. **Replace `DataSource.database(db)`** with
    `DataSource.collection(collection)`.
17. **Replace `InvalidJsonException`** with `FleeceException`.
18. **Remove `TracingDelegate` usage** -- the tracing API has been removed from
    the public API.
19. **Run your app** -- native libraries will be built automatically on first
    run.
