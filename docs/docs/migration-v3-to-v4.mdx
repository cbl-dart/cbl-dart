...
10. **Remove hardcoded revision IDs** -- revision IDs are no longer
    deterministic.
11. **Migrate Database document/index/listener APIs** to use `defaultCollection`
    methods instead.
12. **Migrate ReplicatorConfiguration** -- replace `database` parameter with
    `addCollection`, move `channels`/`documentIds`/filters/resolvers into
    `CollectionConfiguration`.
13. **Migrate Replicator pending document APIs** -- replace
    `pendingDocumentIds`/`isDocumentPending` with
    `pendingDocumentIdsInCollection`/`isDocumentPendingInCollection`.
14. **Replace `DataSource.database(db)`** with
    `DataSource.collection(collection)`.
15. **Replace `InvalidJsonException`** with `FleeceException`.
16. **Remove `TracingDelegate` usage** -- the tracing API has been removed from
    the public API.
17. **Run your app** -- native libraries will be built automatically on first
    run.
