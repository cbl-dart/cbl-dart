---
description: SQL++ for Mobile — Differences from SQL++ for Server
related_content:
...
SQL++ for Mobile's boolean logic rules are based on SQLite's, so:
	...
	- String and Blob are TRUE if the values are casted as a non-zero or FALSE if the values are casted as 0 or 0.0 — see: [SQLITE's CAST and Boolean expressions](https://sqlite.org/lang_expr.html) for more details
	- NULL is FALSE
	- MISSING is MISSING
...
| Operand<br/>a | <br/>b | <br/>a AND b | SQL ++ for Mobile <br/>a OR b |<br />b |<br />a AND b | SQL ++ for Server<br />a OR b | 
| ----| ---- | ---- | ---- | ----- | ----- | ----- | 
| TRUE | TRUE | TRUE | TRUE | - | - | - | 
|  | FALSE | FALSE | TRUE | - | - | - | 
