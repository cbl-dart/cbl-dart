---
description:
  Active-passive peer-to-peer synchronization over WebSockets for Couchbase Lite
  for Dart.
related_content:
  - name: Passive Peer
...
- Supports Wi-Fi and other IP-based network transports over WebSockets.
...
For peer-to-peer sync, that typically maps to:

- `api|ListenerPasswordAuthenticator` on the listener and
  `api|BasicAuthenticator` on the replicator.
- `api|ListenerCertificateAuthenticator` on the listener and
  `api|ClientCertificateAuthenticator` on the replicator.
...
| Field                   | Value                                                                                                                   |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Key Storage**         | Application-defined                                                                                                     |
| **Certificate Storage** | Application-defined                                                                                                     |
| **Notes**               | Persisted `TlsIdentity`s are not currently supported. Use `KeyPair.fromExternal` if you need custom secure key storage. |
...
| **Reference**           | [CNG Key Storage](https://learn.microsoft.com/en-us/windows/win32/seccng/cng-portal) |
